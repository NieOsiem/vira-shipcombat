...
function itemSystem(item) {
  return clone(typeof item?.system?.toObject === "function" ? item.system.toObject() : item?.system);
}
...
function validateProspectiveComponent(item, source) {
  ...
  if (!loadoutValidation.valid) throw validationError("Component would invalidate this ship", loadoutValidation);
}

function optionList(values, selected) {
  return values.map((value) => ({
    value,
    label: value === "" ? "None" : value.replace(/([a-z])([A-Z])/g, "$1 $2").replace(/^./, (letter) => letter.toUpperCase()),
...
export function componentEditDenial(item) {
  ...
  if (embeddedShipInCombat(item)) return "Embedded ship components cannot be edited while their ship is in combat.";
  return "";
}
...
function defaultDefinition(componentClass) {
  switch (componentClass) {
    case "reactor":
      return { nominalOutput: 0, redlineOutput: 0, overclockHeat: 0, recoveryWork: 0 };
    case "drive":
      return { base: { thrust: 0, rotation: 0 }, tiers: [], recoveryWork: 0 };
    case "shield":
      return { topology: "directional", sectors: [...SECTORS], totalBudget: 0, sectorCap: 0, rechargeDelay: 0, tiers: [], recoveryWork: 0 };
    case "sensor":
      return { base: { passiveRange: 0, passiveStrength: 0, activeRange: 0, activeModifier: 0, ewModifier: 0 }, tiers: [], recoveryWork: 0 };
    case "cooling":
    ...
    case "weapon":
      return {
        ...
        signatureSpike: 0,
        recoveryWork: 0,
        readiness: { type: "readyShot", capacity: 1, recovery: "automaticStart", eligibleStarts: 1 },
        traits: [],
...
function tierDefault(componentClass) {
  switch (componentClass) {
    case "drive": return { power: 0, multiplier: 0, online: false };
    case "shield": return { power: 0, online: false, regeneration: 0 };
    case "sensor": return { power: 0, online: false, rangeMultiplier: 0, passiveStrength: 0, activeModifier: 0, ewModifier: 0 };
    case "cooling": return { power: 0, cooling: 0 };
    default: return null;
  }
}
...
function normalizeTiers(form, componentClass, existingTiers = []) {
  return Array.from(form.querySelectorAll("[data-tier-row]")).map((row, index) => {
    const tier = clone(existingTiers[index] ?? tierDefault(componentClass) ?? {});
    ...
    const bool = (name) => row.querySelector(`[data-tier-field="${name}"]`)?.checked === true;
...
function normalizeWeaponTraits(form, existingTraits = []) {
  ...
  for (const trait of existingTraits) {
    ...
    if (supportedIds.includes(id) && !existingById.has(id)) existingById.set(id, trait);
  }

  const replacements = new Map();
  if (checked(form, "trait.nonLethal")) replacements.set("nonLethal", { id: "nonLethal" });
  if (checked(form, "trait.shieldBypass")) {
    const existing = clone(existingById.get("shieldBypass"));
    const channels = ["hull", "heat"].filter((channel) => checked(form, `trait.shieldBypass.${channel}`));
  ...
  if (checked(form, "trait.barrage")) {
    ...
    const profiles = Array.from(form.querySelectorAll("[data-barrage-row]")).map((row, index) => {
      const profile = clone(existingProfiles[index]);
      const read = (name) => {
        const raw = row.querySelector(`[data-barrage-field="${name}"]`)?.value;
        const parsed = Number(raw);
    ...
    replacements.set("barrage", { ...clone(existingById.get("barrage")), id: "barrage", profiles });
  }
  if (checked(form, "trait.vicious")) replacements.set("vicious", { id: "vicious" });
  ...
  for (const id of supportedIds) {
    if (replacements.has(id) && !emitted.has(id)) traits.push(replacements.get(id));
  }
  return traits;
}
...
function normalizeWeaponModes(form, existingModes) {
  ...
  if (!modes.nominal || typeof modes.nominal !== "object") modes.nominal = { overrides: {} };
  ...
  setOptionalNumber(overrides, "powerRating", form, "mode.overclock.powerRating");
  ...
  setOptionalNumber(firingHeat, "amount", form, "mode.overclock.firingHeat.amount");
...
function normalizeWeapon(form, existingDefinition) {
  ...
  readiness.recovery = text(form, "definition.readiness.recovery", "automaticStart");
  setOptionalNumber(readiness, "work", form, "definition.readiness.work");
  setOptionalNumber(readiness, "eligibleStarts", form, "definition.readiness.eligibleStarts");
  ...
  definition.projectileClass = text(form, "definition.projectileClass", "medium");
...
function normalizeDefinition(form, componentClass, existingDefinition) {
  ...
  if (componentClass === "reactor") {
    ...
    definition.sectors = definition.topology === "bubble"
      ? ["bubble"]
      : SECTORS.filter((sector) => checked(form, `definition.sectors.${sector}`));
    ...
    definition.base.passiveStrength = number(form, "definition.base.passiveStrength");
    definition.base.activeRange = number(form, "definition.base.activeRange");
    definition.base.activeModifier = number(form, "definition.base.activeModifier");
...
function normalizeForm(form, forcedClass = null, existingSource = {}) {
  ...
  const componentClass = forcedClass ?? text(form, "system.componentClass", "reactor");
  system.schemaVersion = SCHEMA_VERSION;
  system.componentClass = componentClass;
  system.size = text(form, "system.size", "medium");
  system.driveRole = componentClass === "drive" ? text(form, "system.driveRole") : "";
  system.definition = normalizeDefinition(form, componentClass, system.definition);
  return {
    ...source,
...
function traitById(definition, id) {
  return (definition?.traits ?? []).find((trait) => (typeof trait === "string" ? trait : trait?.id) === id);
}

function viewDefinition(system) {
  const definition = clone(system.definition ?? defaultDefinition(system.componentClass));
  definition.tiers = (definition.tiers ?? []).map((tier, index) => ({ ...tier, index }));
  if (system.componentClass === "weapon") {
    ...
    definition.traitState = {
      ...
      bypassHull: shieldBypass?.channels?.includes?.("hull") || shieldBypass?.hull === true || shieldBypass?.params?.hull === true,
      bypassHeat: shieldBypass?.channels?.includes?.("heat") || shieldBypass?.heat === true || shieldBypass?.params?.heat === true,
      damageShield: shieldBypass ? shieldBypass.damageShield !== false && shieldBypass.params?.damageShield !== false : true,
      barrage: Boolean(barrage),
      vicious: Boolean(vicious),
    };
    definition.barrageProfiles = (barrage?.profiles ?? []).map((profile, index) => ({ ...profile, index }));
...
export class ShipComponentSheet extends HandlebarsApplicationMixin(ItemSheetV2) {
  ...
  get title() {
    return `${this.item?.name ?? this.document?.name ?? "Ship Component"} · Ship Component`;
  }
  ...
  async _prepareContext(options) {
    ...
    return foundry.utils.mergeObject(context, {
      ...
      topologyOptions: optionList(SHIELD_TOPOLOGIES, system.definition.topology),
      isDirectionalShield: componentClass === "shield" && system.definition.topology !== "bubble",
      projectileOptions: optionList(PROJECTILE_CLASSES, system.definition.projectileClass),
      readinessTypeOptions: optionList(READINESS_TYPES, system.definition.readiness?.type),
      readinessRecoveryOptions: optionList(READINESS_RECOVERY, system.definition.readiness?.recovery),
      firingHeatOptions: optionList(FIRING_HEAT_UNITS, system.definition.firingHeat?.per),
      overclockFiringHeatOptions: optionList(FIRING_HEAT_UNITS, system.definition.modes?.overclock?.overrides?.firingHeat?.per),
      sectorOptions: SECTORS.map((sector) => ({ value: sector, label: sector.replace(/^./, (letter) => letter.toUpperCase()), selected: system.definition.sectors?.includes(sector) })),
    }, { inplace: false });
  }
  ...
  _attachPartListeners(partId, html, options) {
    ...
    root.querySelectorAll("[data-row-action]").forEach((button) => {
      button.addEventListener("click", (event) => void this.#rowAction(event, form, button));
    });
  ...
  async #changeClass(form, root, componentClass) {
    try {
      ...
      const draft = normalizeForm(form, previousClass, this.#draftSource(this.item ?? this.document));
      draft.system.componentClass = componentClass;
      draft.system.driveRole = "";
      draft.system.definition = defaultDefinition(componentClass);
      this.#setDraft(draft);
      await this.render();
    } catch (error) {
  ...
  async #changeShieldTopology(form, topology) {
    try {
      this.#assertEditable();
      const draft = normalizeForm(form, null, this.#draftSource(this.item ?? this.document));
      draft.system.definition.topology = topology;
      draft.system.definition.sectors = topology === "bubble" ? ["bubble"] : [...SECTORS];
      this.#setDraft(draft);
      await this.render();
    } catch (error) {
      ui.notifications.error(error?.message ?? String(error));
  ...
  async #rowAction(event, form, button) {
    ...
    try {
      ...
      if (action === "add-tier") {
        ...
        const barrage = draft.system.definition.traits.find((trait) => trait.id === "barrage");
        if (barrage) barrage.profiles.push({ rounds: 1, attackPenalty: 0, maxEffectiveHits: 1 });
