# Blank Hull — Heavy (blank-heavy) — balance report

- Build source: hull file `world-data/hulls/blank-heavy.js` + components file `world-data/components/medium-components.js` through materializeShipConfig
- Components: 36 supplied component sources, 0 installed
- Encounter state: materializeShipConfig + createInitialState — full health (hull 60 of 60, shields ) at the shipped power commit (engines 0 · shields 0 · sensors 0 · cooling 0 · inertia 0 · weapons 0)
...
| --- | --- | --- | --- |
| Reactor | blank-heavy-slot-reactor | reactor / medium | empty |
| Shield | blank-heavy-slot-shield | shield / medium | empty |
| Sensor | blank-heavy-slot-sensor | sensor / medium | empty |
| Cooling | blank-heavy-slot-cooling | cooling / medium | empty |
| Main Drive | blank-heavy-slot-main-drive | drive / medium | empty |
| Reverse Drive | blank-heavy-slot-reverse-drive | drive / medium | empty |
| Port Lateral Drive | blank-heavy-slot-port-lateral-drive | drive / medium | empty |
| Starboard Lateral Drive | blank-heavy-slot-starboard-lateral-drive | drive / medium | empty |
| Inertial Anchor | blank-heavy-slot-inertia | inertia / medium | empty |

| hardpoint | orientation | regions | weapon |
| --- | --- | --- | --- |
| Fore | 0° | fore | empty |
| Port Casemate | -70° | port | empty |
| Starboard Casemate | 70° | starboard | empty |
...
Heat figures assume healthy hardware (fault multipliers are 1). At this commit a full Heat bar (32 Heat Capacity) would clear in — Start(s) of passive cooling alone. Emergency Vent removes nothing at this fit.
...
| orbit 12 mirror | 40 | 0% | — | — | 0 / 0 | 0 / 0 | pool | 0 | 0 | — | — | 0 | 0 | armEvasion:EVASION_ENGINES_UNPOWERED×3200, ping:SENSORS_OFFLINE×3200, acquire:SENSORS_OFFLINE×3200 |
...
| alt orbit 12 mirror | 40 | 0% | — | — | 0 / 0 | 0 / 0 | pool | 0 | 0 | — | — | 0 | 0 | armEvasion:EVASION_ENGINES_UNPOWERED×3200, ping:SENSORS_OFFLINE×3200, acquire:SENSORS_OFFLINE×3200 |
...
report written to tools/balance/blank-heavy.md
